...
void draw_hud(void)
{
    ...
    // Check if anything changed - early exit for maximum performance
    bool player_changed = (prev_player_score != player_score);
    bool enemy_changed = (prev_enemy_score != enemy_score);
    bool game_changed = (prev_game_score != game_score);
    bool level_changed = (prev_game_level != game_level);
    
    if (!player_changed && !enemy_changed && !game_changed && !level_changed) {
        return;  // Nothing changed, skip entire update
    }
    ...
    const int player_index = left_pad;
    const int block_chars = 8;
    
    // Only update player score and bar if changed
    if (player_changed) {
        ...
        // Draw BAR1 (8 chars)
        const int block1_start = player_index + 3 + 1;
    ...
    // Only update game score if changed
    if (game_changed) {
        ...
        const int game_index = player_index + 3 + 1 + 8 + 1;
    ...
    // Only update enemy score and bar if changed
    if (enemy_changed) {
        prev_enemy_score = enemy_score;
        
        // Draw BAR2 (8 chars, filled right-to-left)
        const int block2_start = player_index + 3 + 1 + 8 + 1 + 5 + 1;
        ...
        const int game_index = player_index + 3 + 1 + 8 + 1;
        const int enemy_index = game_index + 5 + 1 + 8 + 1;
    ...
    if (level_changed) {
        ...
        const int game_index = player_index + 3 + 1 + 8 + 1;
        const int enemy_index = game_index + 5 + 1 + 8 + 1;
        const int level_index = enemy_index + 13 + 12;
